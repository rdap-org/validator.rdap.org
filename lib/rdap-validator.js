...
self.testURL = function(url, type, serverType) {
    ...
    self.addMessage("Testing URL is '" + url + "'");
    self.addMessage("Response type is '" + type + "'");
    self.addMessage("Server type is '" + serverType + "'");

    self.lastTestedURL              = url;
    self.lastTestedResponseType     = type;
    self.lastTestedServerType       = serverType;

    const xhr = new self.xhr();
    ...
    xhr.ontimeout = function() {
        self.add(false, "Connection to server timed out after " + xhr.timeout + "ms.");
    };

    xhr.onreadystatechange = function() {
        if (4 == xhr.readyState) {
            if (xhr.status < 1) {
                self.add(false, "Unable to perform request: " + xhr.statusText);

            } else {
                self.testResponse(xhr, url, type, serverType);
 ...
 * @param {string} serverType one of the values in self.serverTypes
 */
self.testResponse = function(xhr, url, type, serverType) {
    ...
    xhr.getAllResponseHeaders().split(/\r\n(?=[^ \t])/).forEach(function(l) {
        const [k, v] = l.split(/:[ \t]+/, 2);
        self.lastTestedResponseHeaders[k.toLowerCase()] = v.replace(/\r\n[ \t]/, ' ');
    });
    ...
    if (!self.add(
        ...
        "Media type is " + self.lastTestedResponseHeaders["content-type"] + "."
    )) return;
    ...
    try {
      ...
      self.add(false, "Response body MUST be valid JSON (error: " + e + ").");
    ...
    self.addMessage("RDAP validation completed with " + self.errors + " error(s).");
};
...
self.validateDomain = function(domain, url, serverType) {
    ...
    self.addMessage("validating domain object...");

    self.validateCommonObjectProperties(domain);
...
self.validateSecureDNS = function(secureDNS) {
    self.addMessage("validating 'secureDNS' object...");
...
self.validateDSData = function(dsData) {
    self.addMessage("validating 'dsData' object...");
...
self.validateKeyData = function(keyData) {
    self.addMessage("validating 'keyData' object...");
...
self.validateEntity = function(entity, url, serverType) {
    self.addMessage("validating entity...");
    ...
    self.validateCommonObjectProperties(entity);
...
self.validateJCard = function (jcard) {
    self.addMessage("validating JCard object...");
...
self.validateNameserver = function(nameserver, url, serverType) {
    self.addMessage("validating nameserver...");
    ...
    self.validateCommonObjectProperties(nameserver);
...
self.validateLink = function(link) {
    self.addMessage("validating link...");
...
self.validateNoticeOrRemark = function (noticeOrRemark) {
    self.addMessage("validating notice/remark...");
...
self.validateEvent = function (event) {
    self.addMessage("validating event...");
...
self.validatePublicId = function (publicId) {
    self.addMessage("validating Public ID...");
 ...
 */
self.validateCommonObjectProperties = function(record) {
    self.addMessage("checking common object properties...");
    ...
    Object.keys(self.commonPropertyValidators).forEach(function(name) {
        ...
        if (record.hasOwnProperty(name)) {
            ...
            self.addMessage("checking '" + name + "' property...");
...
self.validateIPNetwork = function(ipNetwork, url, serverType) {
    ...
    self.addMessage("validating IP network object...");

    self.validateCommonObjectProperties(ipNetwork);
...
self.validateAutnum = function(autnum, url, serverType) {
    ...
    self.addMessage("validating autnum object...");

    self.validateCommonObjectProperties(autnum);
...
self.validateRIRAutnum = function(autnum) {
    self.addMessage("TODO: RIR autnum validation");
};
...
self.validateRIRIPNetwork = function(autnum) {
    self.addMessage("TODO: RIR IP network validation");
};
...
self.validateHelp = function(help, url, serverType) {
    ...
    self.addMessage("validating help response...");
...
self.validateDomainSearch = function(result, url, serverType) {
    ...
    self.addMessage("validating domain search response...");
...
self.validateNameserverSearch = function(result, url, serverType) {
    ...
    self.addMessage("validating nameserver search response...");
...
self.validateEntitySearch = function(result, url, serverType) {
    ...
    self.addMessage("validating entity search response...");
...
self.validateError = function(error, url, serverType) {
    self.addMessage("validating error response...");
...
self.validateGTLDRegistryDomain = function(domain, name) {
    self.addMessage("validating response for a gTLD registry");
...
self.validateGTLDRegistrarDomain = function(domain, name) {
    self.addMessage("validating response for a gTLD registrar");
    self.validateCommonGTLDResponseProperties(domain);
    self.validateCommonGTLDDomainProperties(domain, name);
...
self.validateGTLDNameProperties = function(object, name) {
    if (!self.isULabel(name)) {
        ...
        if (/^xn--/i.test(name) && object.hasOwnProperty("unicodeName")) {
            self.pushPath(".unicodeName");
            self.addMessage("TODO - ensure unicodeName property matches the ldhName");
            self.popPath(".unicodeName");
        }
        ...
        if (object.hasOwnProperty("ldhName")) {
            self.pushPath(".ldhName");
            self.addMessage("TODO - ensure ldhName property matches the unicodeName");
            self.popPath(".ldhName");
        }
...
self.validateGTLDRegistrarEntity = function(rar) {
    self.addMessage("validating entity as a gTLD registrar...");
    ...
    self.addMessage("This tool does not validate the IANA Registrar ID, please refer to https://www.iana.org/assignments/registrar-ids/registrar-ids.xhtml.");
    ...
    if (
        ...
        self.addMessage("TODO - validate link objects once the RDAP Response Profile makes sense.")
    }
...
self.validateGTLDEntity = function(rar) {
    self.addMessage("Validating gTLD registrar response...");
...
self.validateGTLDHelp = function(help) {
    self.addMessage("validating gTLD help response...");
    self.validateCommonGTLDResponseProperties(help);
};

self.validateRIRHelp = function(help) {
    self.addMessage("TODO - validate RIR help response");
};

self.validateGTLDError = function(error) {
    self.addMessage("TODO - validate gTLD error response");
};

self.validateRIRError = function(error) {
    self.addMessage("TODO - validate RIR error response");
};
...
self.validateRIRDomain = function(domain, name) {
    self.addMessage("TODO: RIR domain validation");
};
...
self.validateRIREntity = function(entity) {
    self.addMessage("TODO: RIR entity validation");
};
...
self.validateRIRNameServer = function(nameserver) {
    self.addMessage("TODO: RIR nameserver validation");
};
