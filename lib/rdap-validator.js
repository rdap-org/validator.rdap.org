...
rdapValidator.testResponse = function(xhr, url, type, serverType) {
    ...
    rdapValidator.validateRDAPConformance(record);
    rdapValidator.validateObjectClassName(record, type);

    if (record.hasOwnProperty("notices")) {
        rdapValidator.validateNotices(record.notices);
    }
    ...
    switch (type) {
        case "domain":
            ...
            const name = decodeURI((new URL(url)).pathname.split("/").pop()).normalize("NFC");

            switch (serverType) {
                case "gtld-registry":   rdapValidator.validateGTLDRegistryDomain(record, name); break;
                case "gtld-registrar":  rdapValidator.validateGTLDRegistrarDomain(record, name); break;
                case "rir":             rdapValidator.validateRIRDomain(record, name); break;
            }
        ...
        case "nameserver":
            ...
            switch (serverType) {
                case "gtld-registry":   rdapValidator.validateGTLDNameserver(record); break;
                case "rir":             rdapValidator.validateRIRNameserver(record); break;
            }
        ...
        case "entity":
            ...
            switch (serverType) {
                case "gtld-registry":   rdapValidator.validateGTLDEntity(record); break;
                case "rir":             rdapValidator.validateRIREntity(record); break;
            }

            break;

        case "ip network":
            rdapValidator.validateIPNetwork(record, url, serverType);

            if ("rir" == serverType) rdapValidator.validateRIRIPNetwork(record);

            break;

        case "autnum":
            rdapValidator.validateAutnum(record, url, serverType);

            if ("rir" == serverType) rdapValidator.validateRIRAutnum(record);
        ...
        case "help":
            ...
            switch (serverType) {
                case "gtld-registry":   rdapValidator.validateGTLDHelp(record); break;
                case "gtld-registrar":  rdapValidator.validateGTLDHelp(record); break;
                case "rir":             rdapValidator.validateRIRHelp(record); break;
            }
 ...
 */
rdapValidator.validateRDAPConformance = function(record) {
 ...
 */
rdapValidator.validateNotices = function(notices) {
    rdapValidator.pushPath(".notices");

    if (rdapValidator.addResult(
        rdapValidator.isArray(notices),
 ...
 */
rdapValidator.validateObjectClassName = function(record, type) {
                ...
                rdapValidator.iterate(
                    ...
                    function(v) {
                        switch (p) {
                            ...
                            case "networks":        return rdapValidator.validateIPNetwork(v);
                            case "autnums":         return rdapValidator.validateAutnum(v);
                        }
                    }
                );
            }
 ...
 */
rdapValidator.validateJCardProperty = function(prop) {

    if (!rdapValidator.addResult(
        rdapValidator.isArray(prop),
    ...
    rdapValidator.iterate(funcs, (f) => f(prop));
};

/**
 * Validate a JCard property type.
 */
rdapValidator.validateJCardPropertyType = function(prop) {
    if (rdapValidator.addResult(
        rdapValidator.isString(prop[0]),
        "Item #1 of jCard property MUST be a string."
    )) return;

    if (!property[0].toUpperCase().startsWith("X-")) {
        rdapValidator.addResult(
            rdapValidator.JCardPropertyTypes.includes(property[0].toUpperCase()),
 ...
 */
rdapValidator.validateJCardPropertyParameters = function(property) {
    if (rdapValidator.addResult(
        rdapValidator.isObject(property[1]),
 ...
 */
rdapValidator.validateJCardPropertyValueType = function(property) {
    if (rdapValidator.addResult(
        rdapValidator.isString(property[2]),
        "Item #3 of jCard property MUST be a string."
    )) {
        rdapValidator.addResult(
            rdapValidator.JCardValueTypes.includes(property[2].toUpperCase()),
            "Value type '" + property[2] + "' MUST be present in the IANA registry."
        );
    };

    if (rdapValidator.propertyValues.hasOwnProperty(property[0].toUpperCase())) {
        rdapValidator.addResult(
            rdapValidator.propertyValues[property[0].toUpperCase()].includes(property[3].toLowerCase()),
            "Value '" + property[3] + "' MUST be one of: [" + rdapValidator.propertyValues[property[0].toUpperCase()].join("|") + "]"
        );
    }
};
 ...
 */
rdapValidator.validateJCardPropertyValue = function(property) {
    if (
        "ADR" === property[0].toUpperCase()
        &&
        rdapValidator.isArray(property[3])
    ) {
        rdapValidator.addResult(
            7 == property[3].length,
 ...
 * @param {string} serverType
 */
rdapValidator.validateIPNetwork = function(ipNetwork, url, serverType) {
 ...
 * @param {string} serverType
 */
rdapValidator.validateAutnum = function(autnum, url, serverType) {
 ...
 * @param {null|string} url
 */
rdapValidator.validateRIRAutnum = function(autnum) {
 ...
 * @param {null|string} url
 */
rdapValidator.validateRIRIPNetwork = function(autnum) {
    rdapValidator.addMessage("TODO: RIR IP network validation");
};
 ...
 */
rdapValidator.validateCommonGTLDResponseProperties = function(response) {

    if (response.hasOwnProperty("rdapConformance") && rdapValidator.isArray(response.rdapConformance)) {
        rdapValidator.pushPath(".rdapConformance");
            ...
            if (entity.hasOwnProperty("vcardArray") && rdapValidator.isArray(entity.vcardArray) && rdapValidator.isArray(entity.vcardArray[1])) {
                ...
                rdapValidator.iterate(entity.vcardArray[1], function(p) {
                    if ("ADR" == p[0].toUpperCase()) {
                        ...
                        if (
                            rdapValidator.addResult(
                                rdapValidator.isArray(p[3]),
                                "Value of an 'ADR' property MUST be an array."
                            ) &&
                            7 == p[3].length
                        ) {
                            rdapValidator.pushPath("[6]");
                            ...
                            rdapValidator.popPath("[6]");
                        }

                        rdapValidator.popPath("[3]");
                    }
                });

                rdapValidator.popPath(".vcardArray[1]");
            }

            if (entity.hasOwnProperty("roles") && rdapValidator.isArray(entity.roles) && entity.roles.includes("registrant")) {
                rdapValidator.validategTLDRegistrantEntity(entity);
            }
        });
    ...
    if (response.hasOwnProperty("events") && rdapValidator.isArray(response.entities)) {
 ...
 */
rdapValidator.validateGTLDRegistryDomain = function(domain, name) {
    rdapValidator.addMessage("validating response for a gTLD registry");
    rdapValidator.validateCommonGTLDResponseProperties(domain);
    rdapValidator.validateCommonGTLDDomainProperties(domain, name);
};

/**
 * Validate a domain object as per the gTLD RDAP profile for a registrar.
 */
rdapValidator.validateGTLDRegistrarDomain = function(domain, name) {
    rdapValidator.addMessage("validating response for a gTLD registrar");
    rdapValidator.validateCommonGTLDResponseProperties(domain);
    rdapValidator.validateCommonGTLDDomainProperties(domain, name);
    ...
    const rant = (
            domain.hasOwnProperty("entities") && rdapValidator.isArray(domain.entities)
            ? domain.entities : []
    ).filter((e) =>
        rdapValidator.isObject(e) &&
        e.hasOwnProperty("roles") &&
        rdapValidator.isArray(e.roles) &&
        e.roles.includes("registrant")
    ).shift();

    rdapValidator.addResult(
        rdapValidator.isObject(rant),
...
rdapValidator.validategTLDRegistrantEntity = function(rant) {
    let properties = [];
    if (
        rdapValidator.isObject(rant) &&
        rant.hasOwnProperty("vcardArray") &&
        rdapValidator.isArray(rant.vcardArray) &&
        rdapValidator.isArray(rant.vcardArray[1])
    ) {
        properties = rant.vcardArray[1]
            .filter((p) => rdapValidator.isArray(p))
 ...
 */
rdapValidator.validateCommonGTLDDomainProperties = function(domain, name) {
    ...
    if (!rdapValidator.isULabel(name)) {
        rdapValidator.pushPath(".ldhName");

        if (rdapValidator.addResult(
            domain.hasOwnProperty("ldhName"),
            "A-label domain response MUST include an 'ldhName' property."
        )) {
            rdapValidator.addResult(
                name.toLowerCase() === domain.ldhName.toLowerCase(),
                "The 'ldhName' property MUST match the queried-for domain name."
            );
        }

        rdapValidator.popPath(".ldhName");

        if (/^xn--/i.test(name) && domain.hasOwnProperty("unicodeName")) {
            rdapValidator.pushPath(".unicodeName");
            rdapValidator.addMessage("TODO - ensure unicodeName property matches the ldhName");
            rdapValidator.popPath(".unicodeName");
        }
        ...
        if (rdapValidator.addResult(
            domain.hasOwnProperty("unicodeName"),
            "U-label domain response MUST include an 'unicodeName' property."
        )) {
            rdapValidator.addResult(
                name.toLowerCase() === domain.unicodeName.toLowerCase(),
                "The 'unicodeName' property MUST match the queried-for domain name."
            );
        }

        if (domain.hasOwnProperty("ldhName")) {
            rdapValidator.pushPath(".ldhName");
            rdapValidator.addMessage("TODO - ensure ldhName property matches the unicodeName");
            rdapValidator.popPath(".ldhName");
        }
    ...
    rdapValidator.iterate(
        ...
        function (e) {
            if (
                ...
                rdapValidator.validateGTLDRegistrarEntity(e);
            }
        }
    );
    ...
    let seen = {};
    if (domain.hasOwnProperty("events") && rdapValidator.isArray(domain.events)) {
        domain.events.forEach(function (e) {
            if (rdapValidator.isObject(e) && e.hasOwnProperty("eventAction")) {
                seen[e.eventAction] = 1;
            }
        });
    }
    ["registration", "expiration"].forEach((e) => rdapValidator.addResult(
    ...
    if (
        ...
        &&
        rdapValidator.isArray(domain.notices)
    ) {
        rdapValidator.addResult(
            1 === domain.notices.filter((n) =>
                    rdapValidator.isObject(n) &&
                    "Status Codes" === n.title &&
                    n.hasOwnProperty("links") &&
                    rdapValidator.isArray(n.links) &&
                    n.links.filter((l) =>
                        rdapValidator.isObject(l) &&
    ...
    if (
        ...
        rdapValidator.iterate(
            domain.nameservers,
            rdapValidator.validateGTLDNameserver
        );
    }
    ...
    if (
        ...
        &&
        rdapValidator.isObject(domain.secureDNS)
        &&
        rdapValidator.addResult(
...
rdapValidator.validateGTLDRegistrarEntity = function(rar) {
    ...
    if (rar.hasOwnProperty("entities") && rdapValidator.isArray(rar.entities)) {

        let abuse;
        for (var i = 0 ; i < rar.entities.length ; i++) {
            if (
                rdapValidator.isObject(rar.entities[i]) &&
                rar.entities[i].hasOwnProperty("roles") &&
                rdapValidator.isArray(rar.entities[i].roles) &&
        ...
        if (rdapValidator.addResult(
            ...
            if (
                ...
                rdapValidator.isArray(abuse.vcardArray) &&
                rdapValidator.isArray(abuse.vcardArray[1])
            ) {
                rdapValidator.pushPath(".vcardArray[1]");
                ...
                ["TEL", "EMAIL"].forEach((t) => rdapValidator.addResult(
    ...
    if (
        ...
        &&
        rdapValidator.isArray(rar.links)
    ) {
        rdapValidator.addMessage("TODO - validate link objects once the RDAP Response Profile makes sense.")
    }

    rdapValidator.popPath(".links");
};

rdapValidator.validateGTLDNameserver = function(nameserver) {
    if (rdapValidator.isObject(nameserver)) {
        rdapValidator.pushPath(".ldhName");

        rdapValidator.addResult(
            nameserver.hasOwnProperty("ldhName"),
            "Nameserver MUST have an 'ldhName' property."
        );

        rdapValidator.popPath(".ldhName");
    }
};
rdapValidator.validateGTLDEntity = function(rar) {
    rdapValidator.addMessage("Validating gTLD registrar response...");

    if (rar.hasOwnProperty("roles") && rdapValidator.isArray(rar.roles)) {
        rdapValidator.pushPath(".roles");
    ...
    if (
        ...
        rdapValidator.isArray(rar.vcardArray) &&
        rdapValidator.isArray(rar.vcardArray[1])
    ) {
        rdapValidator.pushPath(".vcardArray[1]");
        ...
        rdapValidator.iterate(
            rar.vcardArray[1],
            function (p) {
                if (rdapValidator.isArray(p) && rdapValidator.isString(p[1])) {
                    seen_types[p[0].toUpperCase()] = 1;

                    if ("ADR" == p[0].toUpperCase()) {
                        rdapValidator.pushPath("[1]");

                        if (rdapValidator.isObject(p[1])) {
                            ...
                            if (rdapValidator.addResult(
                                ...
                                rdapValidator.addResult(
                                    rdapValidator.countryCodes.includes(p[1][k]),
                        ...
                        if (rdapValidator.isArray(p[3])) {
                            rdapValidator.pushPath("[3]");

                            rdapValidator.addResult(
                                (
                                    (
                                        rdapValidator.isString(p[3][2]) &&
                                        p[3][2].length > 0
                                    ) ||
                                    (
                                        rdapValidator.isArray(p[3][2]) &&
                            ...
                            rdapValidator.addResult(
                                rdapValidator.isString(p[3][3]) && p[3][3].length > 0,
        ...
        ["FN", "ADR", "TEL", "EMAIL"].forEach((p) => rdapValidator.addResult(
...
rdapValidator.validateGTLDHelp = function(help) {
    rdapValidator.addMessage("validating gTLD help response...");
    rdapValidator.validateCommonGTLDResponseProperties(domain);
};

rdapValidator.validateRIRHelp = function(help) {
    rdapValidator.addMessage("TODO - validate RIR help response");
};

/**
 * Validate an RIR domain response.
 */
rdapValidator.validateRIRDomain = function(domain, name) {
 ...
 */
rdapValidator.validateRIREntity = function(entity) {
 ...
 */
rdapValidator.validateRIRNameServer = function(nameserver) {
 ...
 */
rdapValidator.isULabel = function(label) {
...
rdapValidator.serverTypes = {
    "vanilla": "Vanilla",
    "gtld-registry": "gTLD registry (February 2024 version RDAP profile)",
    "gtld-registrar": "gTLD registrar (February 2024 version RDAP profile)",
    "rir": "RIR",
};
