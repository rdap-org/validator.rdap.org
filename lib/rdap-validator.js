 ...
 * @param {mixed} context
 */
rdapValidator.resultCallback = (result, message, context) => null;
...
rdapValidator.validateDomain = function(domain, url, serverType) {
    ...
    if (serverType === "thin-gtld-registry") {
        rdapValidator.validateThinGTLD(domain);

    } else if (["thick-gtld-registry", "gtld-registrar"].indexOf(serverType) >= 0) {
