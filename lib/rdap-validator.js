...
rdapValidator.testResponse = function(xhr, url, type, serverType) {
    ...
    switch (type) {
        case "domain":
            ...
            const name = urlobj.pathname.substr(urlobj.pathname.lastIndexOf("/")+1);

            switch (serverType) {
                case "thin-gtld-registry":  rdapValidator.validateThinGTLDDomain(record, name); break;
                case "thick-gtld-registry": rdapValidator.validateThickGTLDDomain(record, name); break;
        ...
        case "ip network":
            ...
            if ("rir" == serverType) rdapValidator.validateRIRIP(record);
        ...
        case "nameserver":
            ...
            switch (serverType) {
                ...
                case "thick-gtld-registry":
                    rdapValidator.validateGTLDNameserver(record);
                    break;

                case "rir":
                    rdapValidator.validateRIRNameserver(record);
                    break;
            }
        ...
        case "entity":
            ...
            switch (serverType) {
                case "thin-gtld-registry":
                case "thick-gtld-registry":
                     rdapValidator.validateGTLDEntity(record);
                     break;

                case "rir":
                    rdapValidator.validateRIREntity(record);
                    break;
            }
 ...
 * @param {null|string} url
 */
rdapValidator.validateRIRIP = function(autnum) {
 ...
 */
rdapValidator.validateThinGTLDDomain = function(domain, name) {
    ...
    rdapValidator.validateCommonGTLDDomain(domain, name);
};

/**
 * Validate a domain object as per the gTLD RDAP profile for a thick registry.
 */
rdapValidator.validateThickGTLDDomain = function(domain, name) {
 ...
 */
rdapValidator.validateCommonGTLDDomain = function(domain, name) {
};
